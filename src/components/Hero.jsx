...
const Hero = () => {
  ...
  return (
    <div className="w-full bg-cover bg-center py-0 md:py-0 flex flex-col relative overflow-hidden md:gap-8 gap-1" style={{ backgroundImage: `url(${Heroimg})` }}>
      ...
      {/* ---------------- SIDEBAR ---------------- */}
      <div className={`fixed top-0 right-0 z-50 h-full w-[280px] bg-[#00423D] transform transition-transform duration-300 ease-in-out ${openSidebar ? "translate-x-0" : "translate-x-full"}`}>
      ...
      {/* ---------------- Hero section ---------------- */}
      <div className="max-w-[1440px] w-full mx-auto p-2 pl-6 z-10 mb-20 md:mb-0">
        <div className="flex flex-col gap-2 max-w-sm sm:max-w-md md:max-w-2xl md:w-full md:pt-10">
          ...
          <motion.p initial={{ y: -30, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.5 }} className="hidden md:block text-[14px] md:text-[16px] font-dmsans font-light max-w-md text-[#F1F1F1] mb-10">
